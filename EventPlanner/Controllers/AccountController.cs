    ...
    public class AccountController : Controller
    {
                        ...
                        case SignInStatus.Success:
                            {
                                if (this.RoleManager.Roles.First().Id == this.UserManager.FindByEmail(model.Email).Roles.First().RoleId)
                                {
                                    ...
                                    return RedirectToAction("IndexAdmin", "Event");
                                }
                                else
                                {
                                    return RedirectToAction("IndexUser", "Event");
                                }
                            }
        ...
        public async Task<ActionResult> Register(RegisterViewModel model)
        {
            if (ModelState.IsValid)
            {
                ...
                var result_user = this.UserManager.Create(user, model.Password);
                ...
                {
                    var state = await this.UserManager.AddToRoleAsync(this.UserManager.FindByName(user.UserName).Id, this.RoleManager.FindById(model.RoleId).Name);
                    if (!state.Succeeded)
                    {
